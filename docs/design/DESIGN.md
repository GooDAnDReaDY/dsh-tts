...
- 2026-09-22 — Windows Child Process Window Suppression (#1, Gitea #135): All child process spawn calls in lib/ (lib/local.js, lib/engines/f5.js, lib/updater.js) pass windowsHide: true to prevent command prompt / console windows (conhost.exe) from flashing on screen for every spoken sentence when running inside GUI hosts (DSH Desktop / Electron) on Windows.
