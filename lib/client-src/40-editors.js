    ...
    function PluginUpdaterBlock(props) {
      ...
      const checkUpdate = React.useCallback(async () => {
        ...
        setMsg(null)
        try {
          const res = await fetch('/api/dsh-tts/update', {
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement('span', { className: 'dts-badge ' + (updateAvailable ? 'dts-badge-warn' : 'dts-badge-on') },
              updateAvailable ? 'v' + latestVer + ' available' : 'v' + currentVer + ' (' + t('updaterUpToDate') + ')'
            )
          ),
          React.createElement(
            'div',
            ...
            updateAvailable
              ? React.createElement(
                  'button',
                  {
                    type: 'button',
                    className: 'dts-btn',
                    style: { borderColor: 'var(--dsw-alias-state-warning-primary)', color: 'var(--dsw-alias-state-warning-primary)' },
                    onClick: onUpdateNow,
