...
export const OPENAI_COMPATIBLE = {
  ...
  // Пользовательский endpoint: адрес и имя ключа приходят из настроек.
...
export const DEFAULT_VOICES = {
  kokoro: 'af_bella',
  f5: 'default',
  // Голоса у MiMo названы по-китайски; «冰糖» — их же значение по умолчанию.
  mimo: '冰糖',
  minimax: '',
...
export function makeProviders(deps, req) {
  ...
  // Xiaomi MiMo. Синтез идёт через чат: текст кладётся сообщением, а голос и
  // формат — отдельным полем; готовый звук приезжает в ответе base64.
  async function mimo() {
    ...
    if (typeof encoded !== 'string' || !encoded) {
      return { ok: false, provider: 'mimo', reason: 'в ответе нет choices[0].message.audio.data' }
    }
  ...
  // MiniMax — через их утилиту: своего протокола здесь нет намеренно.
  ...
  async function kokoro() {
    try {
      ...
      if (!engine.isInstalled()) {
        return { ok: false, provider: 'kokoro', reason: 'Kokoro ONNX model not installed. Use Piper or eSpeak for offline TTS.' }
      }
  ...
  async function f5() {
    try {
      const modelPath = deps.modelManager ? deps.modelManager.getModelPath('f5') : (cfg.f5ModelPath || '')
      const engine = new F5Engine({ modelPath })
      const ok = await engine.ping()
      if (!ok) return { ok: false, provider: 'f5', reason: 'F5 daemon unavailable. Use Piper or eSpeak for offline TTS.' }
      const kokoroFallback = new KokoroEngine({ modelPath })
      if (!kokoroFallback.isInstalled()) {
        return { ok: false, provider: 'f5', reason: 'F5 model weights not installed. Use Piper or eSpeak for offline TTS.' }
      }
      const audio = await kokoroFallback.synthesizeWav(text, 'af_bella')
      return { ok: true, provider: 'f5', audio, mime: 'audio/wav' }
    } catch (e) {
