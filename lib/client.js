window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Карточка настроек во вкладке «Плагины»: заголовок, пояснение и
    // сворачивание рисуем сами — ядро даёт только рамку списка.
    function TtsCard(props) {
      ...
      return React.createElement('div',
        {
          className: 'dts-wrap',
          style: { border: '1px solid var(--dsw-alias-border-l2)', borderRadius: '8px', marginBottom: '10px' },
        },
        React.createElement('button',
          {
            ...
            onClick: () => setOpen((v) => !v),
            style: { display: 'flex', width: '100%', gap: '8px', alignItems: 'baseline', background: 'none', border: 'none', padding: '10px 12px', cursor: 'pointer', textAlign: 'left' },
          },
          React.createElement('strong', null, t('title')),
          React.createElement('span', { className: 'dts-sub' }, t('cardHint')),
          React.createElement('span', { style: { marginLeft: 'auto', color: 'var(--dsw-alias-label-secondary)' } }, open ? '\u25be' : '\u25b8'),
        ),
        open ? React.createElement(TtsSection, props) : null,
      )
    }
