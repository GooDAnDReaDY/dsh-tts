window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SET_CSS =
      ...
      '.dts-btn-danger{color:var(--dsw-alias-state-error-primary);border-color:rgba(239,68,68,0.3)}' +
      '.dts-btn-danger:hover:not(:disabled){background:rgba(239,68,68,0.12);border-color:rgba(239,68,68,0.5)}' +
      ...
      '.dts-badge-on{color:var(--dsw-alias-state-success-primary);border-color:var(--dsw-alias-state-success-primary);background:rgba(16,185,129,0.08)}' +
      '.dts-badge-warn{color:var(--dsw-alias-state-warning-primary);border-color:var(--dsw-alias-state-warning-primary);background:rgba(245,158,11,0.08)}' +
      '.dts-badge-bad{color:var(--dsw-alias-state-error-primary);border-color:var(--dsw-alias-state-error-primary);background:rgba(239,68,68,0.08)}' +
      ...
      '.dts-alert-err{background:rgba(239,68,68,0.1);color:var(--dsw-alias-state-error-primary);border:1px solid rgba(239,68,68,0.2)}' +
      '.dts-alert-warn{background:rgba(245,158,11,0.1);color:var(--dsw-alias-state-warning-primary);border:1px solid rgba(245,158,11,0.2)}' +
      '.dts-alert-ok{background:rgba(16,185,129,0.1);color:var(--dsw-alias-state-success-primary);border:1px solid rgba(16,185,129,0.2)}' +
    ...
    function playAudio(item) {
      return new Promise((resolve) => {
        try {
          ...
          audio.onerror = done
          audio.play().catch(() => {
    // Autoplay without a user gesture is blocked — not a bug; just no sound.
            done()
          })
        } catch (cannotDecode) {
          currentAudioResolve = null
    ...
    function stopPlayback() {
      for (const item of player.queue) {
        if (item && item.prepared && item.prepared.url) {
          try { URL.revokeObjectURL(item.prepared.url) } catch (_) {}
          item.prepared = null
        }
      }
      ...
      if (currentAudioResolve) {
        ...
        try { player.audio.pause() } catch (already) { /* стоит */ }
        player.audio = null
      }
    ...
    function LocalEnginesEditor(props) {
      ...
      const install = (engine) => {
        setLoading(true)
        fetch('/dsh-tts/models/install', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ engine }),
        }).then(() => loadStatus()).finally(() => setLoading(false))
      }
      ...
      return React.createElement('div', { className: 'dts-block' },
        ...
        props.boolField('enableLocalEngines', t('enableLocalEngines'), t('enableLocalEnginesHint')),
        props.boolField('streamingEnabled', t('streamingEnabled'), t('streamingEnabledHint')),
        React.createElement('div', { className: 'dts-entry' },
          React.createElement('div', { className: 'dts-row' },
            React.createElement('span', { className: 'dts-h', style: { minWidth: '150px' } }, 'Kokoro-82M (CPU)'),
            React.createElement('span', {
              className: 'dts-badge' + (kokoro.installed ? ' dts-badge-on' : ''),
            }, kokoro.downloading ? `${t('downloading')} ${kokoro.progress}%` : (kokoro.installed ? t('installed') : t('notInstalled'))),
            !kokoro.installed && !kokoro.downloading ? React.createElement('button', {
              type: 'button', className: 'dts-save', disabled: !props.writable || loading,
              onClick: () => install('kokoro'),
            }, t('installKokoro')) : null,
            kokoro.installed ? React.createElement('button', {
              type: 'button', className: 'dts-link', disabled: !props.writable || loading,
              onClick: () => remove('kokoro'),
            }, t('deleteModel')) : null,
          ),
          kokoro.installed ? props.boolField('kokoroEnabled', t('useKokoro'), t('useKokoroHint')) : null,
        ),
        React.createElement('div', { className: 'dts-entry' },
          React.createElement('div', { className: 'dts-row' },
            React.createElement('span', { className: 'dts-h', style: { minWidth: '150px' } }, 'F5-TTS (GPU)'),
            React.createElement('span', {
              className: 'dts-badge' + (f5.installed ? ' dts-badge-on' : ''),
            }, f5.downloading ? `${t('downloading')} ${f5.progress}%` : (f5.installed ? t('installed') : t('notInstalled'))),
            !f5.installed && !f5.downloading ? React.createElement('button', {
              type: 'button', className: 'dts-save', disabled: !props.writable || loading,
              onClick: () => install('f5'),
            }, t('installF5')) : null,
            f5.installed ? React.createElement('button', {
              type: 'button', className: 'dts-link', disabled: !props.writable || loading,
              onClick: () => remove('f5'),
            }, t('deleteModel')) : null,
          ),
          f5.installed ? props.boolField('f5Enabled', t('useF5'), t('useF5Hint')) : null,
        ),
      )
    }
