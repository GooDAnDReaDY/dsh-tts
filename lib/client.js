window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function TtsSection(props) {
      ...
      // Статус снимка настроек важнее самого значения:
      //   loading      — ответа хоста ещё нет;
      //   unavailable  — хост ответил, но пространство настроек пока не готово;
      //   ready        — значения на месте.
      // При unavailable поле writable берётся из документа и остаётся true,
      // поэтому без проверки статуса карточка рисует пустую, но с виду рабочую форму.
      ...
      return React.createElement('div', { className: 'dts-block' },
        ...
        !installed ? React.createElement('div', {
          style: {
            padding: '10px 14px',
            borderRadius: '8px',
            border: '1px solid var(--dsw-alias-state-warning-primary, #eab308)',
          ...
          React.createElement('span', { style: { fontWeight: 600, color: 'var(--dsw-alias-state-warning-primary, #eab308)' } },
