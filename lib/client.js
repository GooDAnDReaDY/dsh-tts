window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SET_CSS =
      ...
      // Plugin-settings card matches the core card pattern; theme variables only.
    ...
    function TtsSection(props) {
      ...
      const scope = (ctx && ((ctx.get && ctx.get('lanSettings')) || ctx.settingsScope))
        ? ((ctx.get && ctx.get('lanSettings')) || ctx.settingsScope).bind({ namespace: NS })
        : null
      ...
      React.useEffect(() => {
        ...
        let alive = true
        const render = () => { if (alive && scope.getSnapshot) setSnap(scope.getSnapshot()) }
        render()
        if (scope.subscribe) {
    ...
    // looks like a working empty form.
      ...
      // The card header draws the title — do not duplicate a second title block.
