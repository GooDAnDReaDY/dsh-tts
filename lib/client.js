window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SET_CSS =
      ...
      '.dts-alert-ok{background:color-mix(in srgb, var(--dsw-alias-state-success-primary) 10%, transparent);color:var(--dsw-alias-state-success-primary);border:1px solid rgba(16,185,129,0.2)}' +
    ...
    const ru = {
      ...
      'recent': 'Недавние',
      'recentEmpty': 'Пока нет реплик',
      'retry': 'Повторить',
      'advancedCollapsed': 'Дополнительно',
      'confirmClearCache': 'Очистить кэш синтеза? Действие необратимо.',
      'confirmDeleteModel': 'Удалить скачанные веса движка?',
      'autoplayBlocked': 'Звук заблокирован — один раз кликните или коснитесь экрана',
      'enableSound': 'Включить звук',
      'runtimeNotBundled': 'Рантайм нейросинтеза не входит в пакет',
      'runtimeNotBundledHint': 'Установка весов не включает речь. Для offline используйте Edge, Piper или eSpeak.',
      'testProvider': 'Проверить провайдера',
      'breakerOpen': 'Переключатель открыт',
    ...
    function TtsSection(props) {
      ...
      const [advancedOpen, setAdvancedOpen] = React.useState(() => {
        try { return localStorage.getItem('dsh-tts/advancedOpen') === '1' } catch { return false }
      })
