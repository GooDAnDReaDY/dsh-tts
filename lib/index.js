...
import { applyPronunciation, assistantText, speechPhrases, splitSentences, stripForSpeech } from './text.js'
...
export const Config = z.object({
  ...
  longReply: z.enum(['truncate', 'summarize', 'full']).default('truncate')
  ...
  roles: z.object({
    reply: RoleOverride.default({ provider: '', model: '', voice: '', chime: '' }),
    approval: RoleOverride.default({ provider: '', model: '', voice: '', chime: '' }),
    error: RoleOverride.default({ provider: '', model: '', voice: '', chime: '' }),
  }).default({ reply: {}, approval: {}, error: {} })
    .description('Per-role overrides; empty fields inherit the main chain.'),
...
export function apply(ctx, config) {
  ...
  function cleanText(raw, cfg) {
    return applyPronunciation(stripForSpeech(raw, cfg.maxChars, cleanOpts(cfg)), cfg.pronunciation, cfg.language)
  }
  ...
  async function synthesize(rawText, cfg, signal, role = 'reply') {
    ...
    if (cacheAllowed) {
      for (const provider of order) {
        const hit = await speechCache.get(cacheKey([text, provider, models[provider], voices[provider], role]))
        if (hit) return { provider, mime: hit.mime, audio: hit.audio, tookMs: 0, cached: true }
      }
    }
    ...
    if (cacheAllowed) {
      ...
      speechCache.put(cacheKey([text, p, models[p], voices[p], role]), out.mime, out.audio).catch(() => {})
    }
    return out
  }
  ...
  function speakPiece(sid, text, cfg, kind) {
    if (!text) return
    const role = kind === 'notice' ? 'approval' : kind === 'error' ? 'error' : 'reply'
