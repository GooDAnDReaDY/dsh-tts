...
export function apply(ctx, config) {
  ...
  // Дедупликация параллельных запросов на синтез (защита от Cache Stampede)
    ...
    handler: async (req, res) => {
      ...
      if (!voiceInstalled) {
        try {
          ...
          const rootNm = path.join(process.cwd(), 'node_modules', '@goodandready', 'dsh-voice')
          if (fs.existsSync(rootDev) || fs.existsSync(rootNm)) {
            voiceInstalled = true
          }
        } catch {}
      }
      ...
      if (!messengerInstalled) {
        try {
          ...
          const rootNm = path.join(process.cwd(), 'node_modules', '@goodandready', 'dsh-messenger-gateway')
          if (fs.existsSync(rootDev) || fs.existsSync(rootNm)) {
