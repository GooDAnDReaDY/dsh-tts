...
import { assistantText, speechPhrases, splitSentences, stripForSpeech } from './text.js'
...
export function apply(ctx, config) {
  ...
  async function synthesize(rawText, cfg, signal) {
    const text = stripForSpeech(rawText, cfg.maxChars, cleanOpts(cfg))
    ...
    if (cacheAllowed) {
      for (const provider of order) {
        const hit = await speechCache.get(cacheKey([text, provider, models[provider], voices[provider]]))
    ...
    if (cacheAllowed) {
      ...
      speechCache.put(cacheKey([text, p, models[p], voices[p]]), out.mime, out.audio).catch(() => {})
    }
    return out
  }
  ...
  ctx.effect(() => ctx.on('session/event', (session, event) => {
    ...
    if (event.type === 'assistant/message') {
      ...
      if (cfg.speakAsItGoes) {
        const ready = stripForSpeech(text, cfg.maxChars, cleanOpts(cfg))
    ...
    const text = stripForSpeech(raw, cfg.maxChars, cleanOpts(cfg))
    if (!text) return
    const id = `u${++utteranceSeq}`
    const controller = new AbortController()
    const timer = setTimeout(() => controller.abort(), cfg.timeoutMs)
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const text = stripForSpeech(payload.text, live().maxChars, cleanOpts(live()))
