...
export function createModelManager({ root = process.env.DSH_HOME || path.join(os.homedir(), '.dsh'), fetchImpl = fetch } = {}) {
  ...
  async function getStatus(engineId) {
    ...
    const lastErr = lastErrors.get(engineId)
    ...
    return {
      ...
      downloading: false,
      failed: !allFilesExist && Boolean(lastErr),
      error: !allFilesExist && lastErr ? lastErr.message : null,
  ...
  async function installModel(engineId, options = {}) {
    ...
    try {
      ...
      const rawMsg = String(err?.message || err || 'Installation failed')
      const sanitized = rawMsg.replace(/https?:\/\/[^\s]+/g, '[url]')
      lastErrors.set(engineId, { message: sanitized, failedAt: Date.now() })
      throw err
    } finally {
      activeDownloads.delete(engineId)
    }
